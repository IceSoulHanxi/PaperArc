 ...
 * <b>运行时版</b> {@code org.bukkit.**}（连同 craftbukkit 实现类），一旦进编译类路径就会把
 * 我们要实现的 paper-api 表面盖掉 —— 所有 paper 新增方法在编译期直接消失。
 *
 * <p>这个源码集不进产物：只有 {@code main} 会被打进 jar（见 common/build.gradle）。
 */
