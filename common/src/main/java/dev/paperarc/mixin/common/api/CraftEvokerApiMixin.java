 ...
 * explicit setter value lives in dev.paperarc.bridge.ApiState and is pushed into
...
public abstract class CraftEvokerApiMixin {
    ...
    private Sheep paperarc$nmsWololoTarget() {
        Object custom = dev.paperarc.bridge.ApiState.get(this, PAPERARC$WOLOLO_KEY, null);
        if (custom instanceof CraftSheep craftSheep) {
            Sheep sheep = craftSheep.getHandle();
            if (!sheep.isRemoved()) {
                return sheep;
            }
            dev.paperarc.bridge.ApiState.remove(this, PAPERARC$WOLOLO_KEY);
        }
        return null;
    }
    ...
    public void setWololoTarget(@Nullable org.bukkit.entity.Sheep sheep) {
        ...
        dev.paperarc.bridge.ApiState.put(this, PAPERARC$WOLOLO_KEY, sheep);
