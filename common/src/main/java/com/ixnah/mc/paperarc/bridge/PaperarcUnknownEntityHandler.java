 ...
 * （实测重命名为 {@code 目标$内部类名$<hash>}），搬完的 InnerClasses/NestHost 属性仍指向
 * 原 mixin 外围类，与目标类互相矛盾 —— 轻则访问外围私有成员时 IllegalAccessError，
 * 重则 {@code IncompatibleClassChangeError}（B1 在 CraftServer$PaperarcPotionBrewer 上实测）。</p>
 */
